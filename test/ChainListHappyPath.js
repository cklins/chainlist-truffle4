...
contract('ChainList',accounts => {
    ...
    it("should let us sell a first article", async () => {
        let chainListInstance = await ChainList.deployed();
        let receipt = await chainListInstance.sellArticle(articleName1, articleDescription1, web3.toWei(articlePrice1, "ether"), { from: seller});
     ...
     it("should let us sell a second article", async () => {
        let chainListInstance = await ChainList.deployed();
        let receipt = await chainListInstance.sellArticle(articleName2, articleDescription2, web3.toWei(articlePrice2, "ether"), { from: seller});
        assert.equal(receipt.logs.length, 1, "one event should have been triggered");
        assert.equal(receipt.logs[0].event, "LogSellArticle", "event should be LogSellArticle");
    ...
    it("should buy an article", async () => {
        ...
        data = await chainListInstance.getNumberOfArticles();
    ...
    //     let chainListInstance = await ChainList.deployed();
    //     let receipt = await chainListInstance.sellArticle(articleName1, articleDescription1, web3.toWei(articlePrice1, "ether"), {
    //         from: seller
    //     });
    //     assert.equal(receipt.logs.length, 1, "one event should have been triggered");
    //     assert.equal(receipt.logs[0].event, "LogSellArticle", "event should be LogSellArticle");
